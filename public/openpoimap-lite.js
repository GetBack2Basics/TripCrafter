      ...
      marker.on('popupopen', function(){
        try{
          ...
          btn.onclick = function(ev){
            ...
            let attempts = 0;
            const maxAttempts = 50; // ~5s total (50 * 100ms)
            const tryOpen = function(){
              attempts++;
              // feedback: disable button while waiting
              try{ btn.disabled = true; btn.classList.add('waiting'); btn.title = 'Opening...'; }catch(e){}
              if(window.openTripFormForEdit && typeof window.openTripFormForEdit === 'function'){
                console.debug('openpoimap-lite: openTripFormForEdit found after', attempts, 'attempts');
                try{ window.openTripFormForEdit(item); }catch(e){ console.warn('openTripFormForEdit failed', e); }
                try{ marker.closePopup(); }catch(e){}
                try{ btn.disabled = false; btn.classList.remove('waiting'); btn.title = 'Edit'; }catch(e){}
                return;
              }
              if(attempts < maxAttempts){
                // log sparsely to avoid console spam
                if(attempts === 1 || attempts % 10 === 0) console.debug('openpoimap-lite: waiting for host openTripFormForEdit (attempt', attempts, ')');
                setTimeout(tryOpen, 100);
                return;
              }
              console.debug('openpoimap-lite: host helper not found after retries, falling back to prompt');
              if(attempts >= maxAttempts){
                try{ btn.disabled = false; btn.classList.remove('waiting'); btn.title = 'Edit'; }catch(e){}
              }
